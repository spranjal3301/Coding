...
{
    int Pivot_element = arr[start];//*Select First Element as Pivot Element
    int count_left = 0;
    for (int i = start + 1; i <= end; i++){
        if (Pivot_element >= arr[i])count_left++;
    }

    int Pivot_Index = start + count_left;
    swap(arr[start], arr[Pivot_Index]);//*Place Pivot element in right place in Sort array
